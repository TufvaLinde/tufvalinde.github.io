(async () => {
  ...
  while (y > earliestY || (y === earliestY && m0 >= earliestM0)) {
    ...
    for (let slot = loopStart; slot < totalCells; slot++) {
      ...
      const isFutureInCurrentMonth = isCurrentMonth && dayIndex > today.getDate();
      console.log(isFutureInCurrentMonth)

      if (isOutsideMonth) {
        cell.classList.add("is-outside");
        dayNum.textContent = "";
        logs.textContent = "";
      } else if (isFutureInCurrentMonth) {
